...
* Регистрация и вход в систему. Пользователь (ученик или преподаватель) может создать учётную запись, указав имя пользователя и пароль. Реализована авторизация: вход по логину и паролю, а также возможность безопасного выхода (логаута). (Все операции с аккаунтом – регистрация, вход, выход, смена пароля – считаются одним функциональным блоком управления аккаунтом.)
...
* Автоматический поиск ответа по документам. На стороне сервера вопрос пользователя анализируется NLP-моделью, и выполняется семантический поиск по базе загруженных официальных документов лицея. Система находит наиболее релевантные фрагменты текстов (например, из Правил внутреннего распорядка, Устава, приказов и т.д.), которые содержат ответ на вопрос.

* Генерация ответа с указанием источников. На основании найденных фрагментов специальная языковая модель формирует связный ответ на вопрос пользователя. Ответ представляет собой короткий текст (несколько предложений) с указанием ссылок на источники: после каждого факта в ответе в квадратных скобках приводится номер документа или фрагмента, подтверждающего этот факт. Это позволяет пользователю проверить достоверность ответа.

* Отображение источников информации. Вместе с ответом приложение отображает список использованных источников – названия документов (и при возможности номера страниц или разделов), откуда взята информация. Пользователь может увидеть, из какого официального документа получены сведения. Например, будет показано: “[S1] Устав лицея, стр. 5”, “[S2] Правила внутреннего распорядка, п.10”. При нажатии на ссылку источник может открываться (например, PDF-документ) для подробного ознакомления.

* Поиск по ключевым словам (тексту). Помимо диалогового режима, реализована возможность классического поиска: пользователь может ввести ключевое слово или фразу, и система найдет все упоминания этого текста в базе документов. Результаты выдаются списком документов и контекстов, где встречается запрос. Этот функционал поможет, если пользователь предпочитает сам просматривать документы, но хочет быстро найти нужное место.

* Загрузка новых документов в базу знаний. Администратор системы (или уполномоченный пользователь) может добавлять новые документы для индексирования. Например, если появляется новое положение или приказ, PDF-файл можно загрузить через интерфейс “Добавить документ”. Система обрабатывает документ: извлекает текст, разбивает на фрагменты и добавляет в поисковый индекс. После этого информация из нового документа становится доступна для поиска и вопросов. (Эта функция будет доступна только определённым ролям – см. требование 9.)

* Сохранение и обновление данных на сервере. Все добавленные документы хранятся в базе данных (или файловом хранилище сервера) – данные не зависят от устройства пользователя. Это означает, что пользователь, зайдя под своим логином на любом устройстве, будет работать со всей актуальной базой документов. Также сохраняются результаты действий: например, история вопросов (см. требование 10) привязана к аккаунту и доступна с любого устройства.

* Разграничение доступа и административные функции. В системе предусмотрены различные уровни доступа. Обычные пользователи (ученики, учителя) могут задавать вопросы и просматривать документы. Администратор (например, сотрудник администрации лицея) имеет дополнительные функции: добавление или удаление документов, управление списком пользователей (при необходимости блокировка или сброс пароля), просмотр статистики использования системы (например, наиболее частые запросы для улучшения базы знаний). Администрирование доступно через отдельный защищённый интерфейс.

* История запросов и экспорт ответов. Система хранит историю предыдущих запросов пользователя и полученных ответов. Пользователь может просмотреть список своих последних вопросов и при необходимости повторно открыть ответ. Также предусмотрена возможность экспорта информации: пользователь может сохранить ответ вместе со ссылками, например, в виде текстового файла или PDF-документа (для печати или отправки). Это позволит, к примеру, ученику сохранить полученную справку или включить её в свой проект с указанием источника.
...
* Высокая сложность проекта и дефицит времени: Проект затрагивает сразу несколько сложных областей (веб + БД + нейросеть). Есть риск не успеть реализовать весь запланированный функционал к дедлайну. Например, может затянуться этап интеграции или отладки (баги в авторизации, проблемы с совместной работой модели и сервера). Меры: строгое следование плану, приоритизация функций. Критически необходимые функции (поиск ответа) делаются в первую очередь; менее важные (например, продвинутая статистика, красивый дизайн) могут быть упрощены или отложены. Регулярные показы кураторам помогут выявить отставание вовремя. При необходимости функционал может быть сужен (по согласованию) – но только в меньших деталях, базовые требования (не менее 9 функций) останутся неприкосновенными.
